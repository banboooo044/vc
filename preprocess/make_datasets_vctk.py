...
# table(ID/AGE/GENDER/ACCENTS/REGION) -> [ID]
def read_speaker_info(speaker_info_path):
    ...
    return speaker_ids # List of ID

# wavファイルの入ったディレクトリ -> dictionary { speakerID : filenameList }
...
if __name__ == '__main__':
    data_dir = sys.argv[1] # dir/wav48
    speaker_info_path = sys.argv[2] # speaker-info.txt
    output_dir = sys.argv[3] # /groups/jjery2243542/data/vctk/trimmed_vctk_spectrograms/sr_24000_mel_norm/
    test_speakers = int(sys.argv[4]) # num of test_speaker : 20(default)
    test_proportion = float(sys.argv[5]) # in-test(validation)の割合 : 0.1(default)
    sample_rate = int(sys.argv[6]) # 24000(default)
    n_utts_attr = int(sys.argv[7]) # 5000(default)


    speaker_ids = read_speaker_info(speaker_info_path) # List [ID of speaker]
    random.shuffle(speaker_ids)

    train_speaker_ids = speaker_ids[:-test_speakers] # ID List of train data
    test_speaker_ids = speaker_ids[-test_speakers:]  # ID List of test data

    speaker2filenames = read_filenames(data_dir) # dic { speakerID : filenameList }
    ...
    for speaker in train_speaker_ids:
        ...
        test_data_size = int(len(path_list) * test_proportion) # in_test_size
    ...
    for dset, path_list in zip(['train', 'in_test', 'out_test'], \
        ...
        for i, path in enumerate(sorted(path_list)):
            ...
            filename = path.strip().split('/')[-1]
            mel, mag = spec_feature_extraction(path) # log(melspectrogram), log(magnitude)
            data[filename] = mel
        ...
        # mean, std: train dataの平均と分散で標準化
        for key, val in data.items():
            val = (val - mean) / std
            data[key] = val

        # 正規化した log(melspectrogram) を {dset}.pkl に格納.
